...
- applicable `AGENTS.md` instructions and their explicit local references;
...
The context has fixed byte, file, catalog, fragment, and omission limits.
...
When all mandatory instructions fit and relevant evidence was selected,
participant tools are disabled for the entire Plan run. If the context is
insufficient or construction fails, only independent authors retain the
existing Seatbelt-protected local read/search policy. The manifest records the
fallback reason.
